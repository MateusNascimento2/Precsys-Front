...
export default function TabelaGeneradaCalculo() {
  ...
  const [showTabelas, setShowTabelas] = useState({
    tabelaResumo: false,
    tabelaCalculo: true,
    tabelaIR: false,
    tabelaCustas: false
  ...
  const [showTabelaCalculoInicial, setShowTabelaCalculoInicial] = useState(true)
  const [showTabelaInicioPeriodoGraca, setShowTabelaInicioPeriodoGraca] = useState(true)
  const [showTabelaInicioTaxaSelic, setShowTabelaInicioTaxaSelic] = useState(true)
  const [showTabelaInicioPeriodoGraca2, setShowTabelaInicioPeriodoGraca2] = useState(true)
  const [showTabelaFinalPeriodoGraca, setShowTabelaFinalPeriodoGraca] = useState(true)
  const [showTabelaAteInicioSelic, setShowTabelaAteInicioSelic] = useState(true)
  const [showTabelaCalculoFinal, setShowTabelaCalculoFinal] = useState(true);
  const [showTabsTabelaCalculo, setShowTabsTabelaCalculo] = useState(true);
  ...
  const handleExportPDF = () => {
    ...
    addTable(
      ...
      [
        ...
        ["Orçamento", orcamento || "-"],
        ["Valor Principal", valorPrincipal.valorFormatado || "-"],
        ["Valor dos Juros", valorJuros.valorFormatado || "-"],
        ["Correção Monetária", correcaoMonetaria.valorFormatado || "-"],
        ["Rioprêvidencia", rioPrevidencia.valorFormatado || "-"],
        ["Data Base", dataBaseStr || "-"],
        ["Data de Atualização", dataAtualizacaoStr || "-"],
    ...
    addTable(
      ["Cálculo Inicial", ''],
      [
        ["Data", calculoInicialData || "-"],
        ["Valor Principal", calculoInicialValorPrincipal || "-"],
        ["Valor Juros", calculoInicialValorJuros || "-"],
        ["Valor Total", calculoInicialValorTotal || "-"],
      ]
    );

    if (showTabelaInicioPeriodoGraca) {
      // Início do Período da Graça
      addTable(
        ["Início do Período da Graça", ''],
        [
          ["Data", inicioPeriodoGracaData || "-"],
          ["Correção Monetária", "-"],
          ["Fator", inicioPeriodoGracaFator || "-"],
          ["Valor Principal Corrigido", inicioPeriodoGracaValorPrincipalCorrigido || "-"],
          ["Valor Juros Corrigido", inicioPeriodoGracaValorJurosCorrigido || "-"],
          ["Valor Total Corrigido", inicioPeriodoGracaValorTotalCorrigido || "-"],
          ["Juros", "-"],
          ["Número Dias", inicioPeriodoGracaNumeroDias || "-"],
          ["Taxa de Juros", inicioPeriodoGracaTaxaJuros || "-"],
          ["Valor Juros", inicioPeriodoGracaValorJuros || "-"],
          ["Valor Atualizado", "-"],
          ["Valor Principal Corrigido", inicioPeriodoGracaAtualizadoValorPrincipalCorrigido || "-"],
          ["Valor Juros Corrigido", inicioPeriodoGracaAtualizadoValorJurosCorrigido || "-"],
          ["Valor Total", inicioPeriodoGracaAtualizadoValorTotal || "-"],
        ]
      );
    }

    if (showTabelaInicioTaxaSelic) {
      // Início Taxa Selic
      addTable(
        ["Início Taxa Selic", ''],
        [
          ["Data", inicioTaxaSelicData || "-"],
          ["Correção Monetária", "-"],
          ["Fator", inicioTaxaSelicFator || "-"],
          ["Valor Principal Corrigido", inicioTaxaSelicValorPrincipalCorrigido || "-"],
          ["Valor Juros Corrigido", inicioTaxaSelicValorJurosCorrigido || "-"],
          ["Valor Total Corrigido", inicioTaxaSelicValorTotalCorrigido || "-"],
          ["Juros", "-"],
          ["Número Dias", inicioTaxaSelicNumeroDias || "-"],
          ["Taxa de Juros", inicioTaxaSelicTaxaJuros || "-"],
          ["Valor Juros", inicioTaxaSelicValorJuros || "-"],
          ["Valor Atualizado", "-"],
          ["Valor Principal Corrigido", inicioTaxaSelicAtualizadoValorPrincipalCorrigido || "-"],
          ["Valor Juros Corrigido", inicioTaxaSelicAtualizadoValorJurosCorrigido || "-"],
          ["Valor Total", inicioTaxaSelicAtualizadoValorTotal || "-"],
        ]
      );
    }

    if (showTabelaInicioPeriodoGraca2) {
      // Início do Período da Graça
      addTable(
        ["Início do Período da Graça", ''],
        [
    ...
    if (showTabelaFinalPeriodoGraca) {
      // Final do Período da Graça
      addTable(
        ["Fim do Período da Graça ou Atualização", ''],
        [
          ["Data", finalPeriodoGracaData || "-"],
          ["Correção Monetária", "-"],
          ["Fator", finalPeriodoGracaFator || "-"],
          ["Valor Principal Corrigido", finalPeriodoGracaValorPrincipalCorrigido || "-"],
          ["Valor Juros Corrigido", finalPeriodoGracaValorJurosCorrigido || "-"],
          ["Valor Total Corrigido", finalPeriodoGracaValorTotalCorrigido || "-"],
        ]
      );
    }

    if (showTabelaAteInicioSelic) {
      // Início Taxa Selic
      addTable(
        ["Até o Início da Selic", ''],
        [
          ["Data", ateInicioSelicData || "-"],
          ["Correção Monetária", "-"],
          ["Fator", ateInicioSelicFator || "-"],
          ["Valor Principal Corrigido", ateInicioSelicValorPrincipalCorrigido || "-"],
          ["Valor Juros Corrigido", ateInicioSelicValorJurosCorrigido || "-"],
          ["Valor Total Corrigido", ateInicioSelicValorTotalCorrigido || "-"],
        ]
      );
    }


    if (showTabelaCalculoFinal) {
      // Cálculo Final
      addTable(
        ["Cálculo Final", ''],
        [
          ["Data", calculoFinalData || "-"],
          ["Correção Monetária / SELIC", "-"],
          ["Fator", calculoFinalFator || "-"],
          ["Valor Principal Corrigido", calculoFinalValorPrincipalCorrigido || "-"],
          ["Valor Juros Corrigido", calculoFinalValorJurosCorrigido || "-"],
          ["Valor Total Corrigido", calculoFinalValorTotalCorrigido || "-"],
        ]
      );
    }
  ...
  function submitForm(event) {
    ...
    if (orcamento === '') {
      return alert('Escolha um Orçamento !')
    }
    ...
    const dataBase = parseDateAsLocal(dataBaseStr);
    const dataAtualizacao = parseDateAsLocal(dataAtualizacaoStr);
    const dataInicioPeriodoDaGraca = parseDateAsLocal(dataInicioPeriodoDaGracaStr);
    const dataVencimentoPeriodoDaGraca = parseDateAsLocal(dataVencimentoPeriodoDaGracaStr);
    ...
    async function calcularValores(dataBase, dataInicioPeriodoDaGraca, dataVencimentoPeriodoDaGraca, valorPrincipal, valorJuros, rioPrevidencia, dataAtualizacao, marco, correcaoMonetaria) {
      ...
      setShowTabsTabelaCalculo(true)
      setShowTabelaCalculoInicial(true)
      console.log("Cálculo Inicial");
      console.log("Data:", new Date(dataBase).toLocaleDateString("pt-BR"));
      setCalculoInicialData(new Date(dataBase).toLocaleDateString("pt-BR"))
      ...
      if (dataInicioPeriodoDaGraca > dataAtualizacao) {
        //Calculo se Periodo da graça for depois da Atualização
        if (dataBase < marco) {
          if (dataAtualizacao < marco) {
            // Cálculo DataBase Até o Periodo da Graça (ipca-E/jUROS)
            let FatorIPCAEDataBase = await PegarDataDoJSON(dataBaseStr, 'fatorNT');
            let FatorIPCAEAtualizacao = await PegarDataDoJSON(dataAtualizacaoStr, 'fatorNT');
            let FatorJurosPoupancaDataBase = await PegarDataDoJSON(dataBaseStr, 'jurosPoupanca');
            let FatorJurosPoupancaAtualizacao = await PegarDataDoJSON(dataAtualizacaoStr, 'jurosPoupanca');


            setShowTabelaInicioPeriodoGraca(true)
            console.log("Início do Período da Graça (", dataAtualizacao.toLocaleDateString("pt-BR"), ")");
            console.log("Data:", dataAtualizacao.toLocaleDateString("pt-BR"));
            setInicioPeriodoGracaData(dataAtualizacao.toLocaleDateString("pt-BR"));

            console.log("\nCorreção Monetária");
            console.log("Fator:", FatorIPCAEDataBase / FatorIPCAEAtualizacao, "Entre as datas: ", dataBase.toLocaleDateString("pt-BR"), " e ", dataAtualizacao.toLocaleDateString("pt-BR"), "");
            setInicioPeriodoGracaFator(FatorIPCAEDataBase / FatorIPCAEAtualizacao)
            ...
            nValorPrincipal = nValorPrincipal * (FatorIPCAEDataBase / FatorIPCAEAtualizacao);
            ...
            nValorJuros = nValorJuros * (FatorIPCAEDataBase / FatorIPCAEAtualizacao);
            ...
            console.log("\nJuros");
            console.log("Número de Dias:", Math.floor((dataAtualizacao - dataBase) / (1000 * 60 * 60 * 24)));
            setInicioPeriodoGracaNumeroDias(Math.floor((dataAtualizacao - dataBase) / (1000 * 60 * 60 * 24)))
            console.log("Taxa de Juros:", (FatorJurosPoupancaDataBase - FatorJurosPoupancaAtualizacao) * 100, "Entre as datas: ", dataBase.toLocaleDateString("pt-BR"), " e ", dataAtualizacao.toLocaleDateString("pt-BR"), "");
            setInicioPeriodoGracaTaxaJuros((FatorJurosPoupancaDataBase - FatorJurosPoupancaAtualizacao) * 100)
            setInicioPeriodoGracaTaxaJurosObs(`Entre as datas: ${dataBase.toLocaleDateString('pt-BR')} e ${dataAtualizacao.toLocaleDateString('pt-BR')}`)
            console.log("Valor Juros: ", nValorPrincipal * (FatorJurosPoupancaDataBase - FatorJurosPoupancaAtualizacao));
            setInicioPeriodoGracaValorJuros((nValorPrincipal * (FatorJurosPoupancaDataBase - FatorJurosPoupancaAtualizacao)).toLocaleString("pt-BR", { style: "currency", currency: "BRL" }))
            nValorJuros = nValorJuros + (nValorPrincipal * (FatorJurosPoupancaDataBase - FatorJurosPoupancaAtualizacao));
            ...
            let FatorSelicAtualizacao = await PegarDataDoJSON(dataAtualizacaoStr, 'taxaSelic');
            ...
            console.log("\nCorreção Monetária / SELIC");
            console.log("Fator:", ((FatorSELICMarco - FatorSelicAtualizacao) / 100) + 1, "Entre as datas:", marco.toLocaleDateString("pt-BR"), " e ", dataAtualizacao.toLocaleDateString("pt-BR"), ")");
            setInicioPeriodoGracaFator(((FatorSELICMarco - FatorSelicAtualizacao) / 100) + 1)
            setInicioPeriodoGracaFatorObs(`Entre as datas: ${marco.toLocaleDateString('pt-BR')} e ${dataAtualizacao.toLocaleDateString('pt-BR')}`)

            nValorPrincipal = nValorPrincipal * (((FatorSELICMarco - FatorSelicAtualizacao) / 100) + 1)
            console.log("Valor Principal Corrigido: ", nValorPrincipal);
            setInicioPeriodoGracaValorPrincipalCorrigido(nValorPrincipal.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }))

            nValorJuros = nValorJuros * (((FatorSELICMarco - FatorSelicAtualizacao) / 100) + 1)
            console.log("Valor Juros Corrigido: ", nValorJuros);
            setInicioPeriodoGracaValorJurosCorrigido(nValorJuros.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }))
          ...
          console.log("\nInício do Período da Graça");
          console.log("Data: ", dataAtualizacao.toLocaleDateString("pt-BR"));
          setInicioPeriodoGracaFator(dataAtualizacao.toLocaleDateString("pt-BR"))

          console.log("\nCorreção Monetária / SELIC");
          console.log("Fator:", ((FatorSELICDataBase - FatorSELICAtualizacao) / 100) + 1, "Entre as datas:", dataBase.toLocaleDateString("pt-BR"), " e ", dataAtualizacao.toLocaleDateString("pt-BR"), ")");
          setInicioPeriodoGracaFator(((FatorSELICDataBase - FatorSELICAtualizacao) / 100) + 1)
          setInicioPeriodoGracaFatorObs(`Entre as datas: ${dataBase.toLocaleDateString('pt-BR')} e ${dataAtualizacao.toLocaleDateString('pt-BR')}`)

          nValorPrincipal = nValorPrincipal * (((FatorSELICDataBase - FatorSELICAtualizacao) / 100) + 1)
          console.log("Valor Principal Corrigido: ", nValorPrincipal);
          setInicioPeriodoGracaValorPrincipalCorrigido(nValorPrincipal.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }))

          nValorJuros = nValorJuros * (((FatorSELICDataBase - FatorSELICAtualizacao) / 100) + 1)
          console.log("Valor Juros Corrigido: ", nValorJuros);
          setInicioPeriodoGracaValorJurosCorrigido(nValorJuros.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }))
        ...
        // Calculo se Periodo da Graça for antes da Atualização
        if (dataBase < marco) {
  ...
  return (
    ...
    <main className="px-2 lg:px-[30px]">
      <div className="w-full mb-[60px] flex flex-col lg:grid lg:grid-cols-[250px_1fr] xl:grid-cols-[380px_1fr] gap-4 lg:relative">
        ...
        >
          <div className="flex flex-col gap-2 lg:gap-2 xl:fixed xl:top-[7%] xl:w-[380px]  dark:border-neutral-600 lg:border-r pb-4 mb-[1px] lg:pb-0 lg:pr-4">
            ...
            <div className='flex flex-col gap-1'>
              ...
              <select className="dark:bg-neutral-800 border rounded dark:border-neutral-600 py-1 px-2 focus:outline-none placeholder:text-[14px] text-gray-400" value={orcamento} onChange={(e) => setOrcamento(e.target.value)}>
        ...
        <div ref={pdfContentRef}>
          ...
          {showTabsTabelaCalculo &&
            <TabsTabelaCalculo showTabelas={showTabelas} setShowTabelas={setShowTabelas} />
          }

          <div className="lg:h-[580px] lg:overflow-y-auto pt-4 lg:p-4">
            <div className={showTabelas.tabelaCalculo ? "flex flex-col justify-center gap-4" : 'hidden'}>
              {/* Calculo Inicial */}
