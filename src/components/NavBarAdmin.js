        ...
        <ul className={auth.user.admin ? 'divide-y divide-neutral-300 dark:divide-neutral-700 flex w-full px-2 flex-col items-center gap-2 text-[13px] lg:flex-row lg:divide-y-0' : 'divide-y divide-neutral-300 dark:divide-neutral-700 flex w-full px-2 flex-col items-start lg:items-center gap-2 text-[13px] lg:flex-row lg:divide-y-0'}>
          ...
          {auth.user.admin ? // NavBar de Admin
            <>
              ...
              <li onClick={() => handleShow('notificacoes')} className='cursor-pointer w-full p-2 lg:px-2 lg:border-0 lg:hover:bg-neutral-100 lg:dark:hover:bg-neutral-800 hover:rounded'>
                <div className='flex justify-between items-center lg:gap-3 '>
                  <span className='font-[500] text-[#666666] dark:text-neutral-300 '>Notificações</span>
                  <span className='text-[12px] dark:text-neutral-300'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={showMenu && menuType === 'notificacoes' ? "w-3 h-3 inline-block rotate-180" : 'w-3 h-3 inline-block'}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                    </svg>
                  </span>
                </div>
                <AnimatePresence>
                  {showMenu && menuType === 'notificacoes' && (
                    <motion.div
                      initial={{ height: 0, opacity: 0 }}
                      animate={{ height: 'auto', opacity: 1 }}
                      exit={{ height: 0, opacity: 0 }}
                    ...
                    >
                      <ul className='lg:ml-0 '>
                        ...
                        <li className='p-2 rounded cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800' onClick={() => handleRoute('/notificacoes/status')}>
                          <span className='font-[600] text-[14px] text-[#171717] dark:text-neutral-300'>Mudanças de Status</span>
                          <p className='font-[500] text-[#666666] dark:text-neutral-500'>Ver todas as Mudanças de Status dos Precatórios</p>
                        </li>
                      </ul>
                    </motion.div>
                  )}
            ...
            : auth.user.advogado ? // NavBar do Advogado
              <>
                <li onClick={() => handleShow('pessoal')} className='cursor-pointer w-full p-2 lg:px-2  lg:border-0 lg:hover:bg-neutral-100 lg:dark:hover:bg-neutral-800 hover:rounded'>
                  <div className='flex justify-between items-center lg:gap-3'>
                    <span className='font-[500] text-[#666666] dark:text-neutral-300 '>Pessoal</span>
