...
export default function PublicacoesDiario() {
  ...
  const [isCheckedFiltroHomologado, setIsCheckedFiltroHomologado] = useState(false)
  ...
  console.log(publicacoes.filter(p => p.ja_homologado === 1))

  useEffect(() => {
    fetchPublicacoes();
  }, [searchQuery, dataInicial, paginaAtual, isCheckedFiltroHomologado]);

  const updateHomologado = async (precatorio, valorHomologado) => {
    try {
      console.log(valorHomologado);
      await axiosPrivate.put(`/homologar-precatorio/${precatorio}`, { isHomologado: valorHomologado });
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const fetchPublicacoes = async () => {
    try {
      ...
      const { data } = await axiosPrivate.get('/publicacoes-com-prazos', {
        ...
        params: {
          ...
          isHomologado: isCheckedFiltroHomologado ? 1 : 0,
        },
      });

      setPublicacoes(data.dados);
      setPublicacoesFiltradas(data.dados);
  ...
  const exportarParaPDF = () => {
    ...
    publicacoes.forEach((pub, index) => {
      ...
      alturaBox = 85 + textoLinhas.length * 14;
  ...
  const handleFiltroHomologar = () => {
    setIsCheckedFiltroHomologado(!isCheckedFiltroHomologado);
                ...
                <div className={'hidden lg:block lg:sticky lg:top-[8%] lg:w-[320px] lg:border-r dark:border-neutral-600 lg:pr-2 lg:mt-4'}>
                  <div className='flex items-center justify-between'>
                    ...
                    <div className='flex items-center gap-1'>
                      ...
                      <button onClick={() => {
                        setDataInicial('')
                        setIsCheckedFiltroHomologado(false)
                  ...
                  <div className="flex flex-col gap-2 py-3 text-neutral-600 dark:text-neutral-400">
                    ...
                    {pathname === '/publicacoes-intimadas' ? <div className="flex justify-between border dark:border-neutral-600 rounded px-2 py-1 items-center">
                      <label className="text-[12px] font-bold">
                        Publicações Homologadas:
                      </label>
                      <div onClick={() => {
                        handleFiltroHomologar()
                      }}
                        className={`size-5 border rounded dark:border-neutral-600 cursor-pointer  hover:bg-neutral-200 ${isCheckedFiltroHomologado ? 'dark:bg-white dark:text-black bg-black text-white dark:hover:bg-neutral-200 hover:bg-neutral-700' : 'dark:hover:bg-neutral-800'}`}>
                        {isCheckedFiltroHomologado ? <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 ml-[2px] mt-[1px]" viewBox="0 0 20 20" fill="currentColor" stroke="currentColor" stroke-width="1"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg> : null}
                      </div>
                    </div> : null}
                ...
                <div className='flex flex-col gap-5 w-full'>
                  <div className='w-full h-full max-h-full'>
                    {isLoading ? (
                      ...
                      :
                      <PublicacoesDiarioList publicacoesFiltradas={publicacoesFiltradas} publicacoesQtd={totalResultados} updateHomologado={updateHomologado} fetchPublicacoes={fetchPublicacoes} />
                    }
                  </div>
