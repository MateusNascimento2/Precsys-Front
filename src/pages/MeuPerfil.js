...
import Cessoes from './AllCessoes';
...
const MeuPerfil = () => {
    ...
    switch (activeItem) {
      ...
      case 'configuracoes':
        return (
          ...
          >
            <ConfiguracoesPerfil setUser={setUser} user={user} id={id} />
          </motion.div>
        );
      case 'ferramentas':
        return (
  ...
  const toggleUserStatus = async () => {
    if (auth.user.admin && !isUpdatingStatus) {
      ...
      try {
        const updatedAtivo = { ativo: user.ativo ? 0 : 1 };
        await axiosPrivate.put(`/users/${user.id}/ativar`, updatedAtivo);
                    ...
                    <div className={"lg:sticky lg:top-[8%]"}>
                      <div className="p-2 lg:p-0 lg:px-2 h-full lg:border-r dark:lg:border-neutral-600 lg:h-[320px]">
                        ...
                        <ul className="flex flex-col mt-4 divide-y flex-wrap dark:divide-neutral-600">
                          ...
                          {auth.user.admin || auth.user.acesso_api ?
                            <li className={"px-4 py-1 lg:py-2"}>
                              <a
